...
输入中文姓名或中文身份线索时，系统会在通用检索之外追加小红书、微信公众号、视频号、百度百科、微博、知乎、Bilibili 与抖音等国内公开来源的定向查询。遇到大学、学院、研究院等学术身份线索时，会优先检索学校和机构官网，并把中文姓名转换为“姓在前”和“名在前”两种拼音顺序，结合英文机构名再检索一轮外文资料。底层仍使用 Brave Search API；不会绕过登录、验证码或平台访问限制。无法读取正文的页面可以作为公开入口呈现，但不会被当作已读证据写进人物观点。
